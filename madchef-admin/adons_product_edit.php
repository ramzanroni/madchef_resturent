  ...
  if (isset($_POST['post_id'])) {
    ...
    if (!empty($_FILES)) 
    {
      ...
      if(in_array($ext, $allwoed_extention)){
        if ($_FILES['attachment']['size'] < 10485760) {
          ...
          $data = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM `adons_product` WHERE `id` = '".$id_update."'"));
          if ($data['attachment'] != "") {
          ...
          $sql = "UPDATE `adons_product` SET `brand`='$brand_update',`branch`='$branch_update',`deatails`='$deatails_update',`name`='$name_update',`category`='$category_update',`price`='$price_update',`vat`='$vat_update',`sd`='$sd_update',`dicount`='$dicount_update',`image`='$newfilename' WHERE `id`='$id_update'";
    ...
    $sql = "UPDATE `adons_product` SET `brand`='$brand_update',`branch`='$branch_update',`deatails`='$deatails_update',`name`='$name_update',`category`='$category_update',`price`='$price_update',`vat`='$vat_update',
  ...
  $id=$_POST['id'];
  $sql = "SELECT * FROM  adons_product WHERE `id`='$id'";
  $result = mysqli_query($con,$sql);
  ...
  <div class="form-group">
    ...
    <div class="col-sm-9">
      <input type="text" class="form-control"name="sd_update" id="sd_update" type="text" value="<?php echo $assoc['sd']; ?>" placeholder="" required="required">
    </div>
  </div>
  <br><br> 
  <div class="form-group">
